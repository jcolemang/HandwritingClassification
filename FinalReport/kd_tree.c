...
/*
 * Important that I can access the pointers to
 * the nodes to modify their values in place
 */
KD_Node*
...
/*
 * The entire reason for using a tree, and in particular a
 * KD-Tree. Checks the points in regions that overlap
 * with the lower and upper bounds, which is usually a
 * whole lot less than all of them.
 */
DynamicArray*
...
/*
 * Probably wont use this but it is a good
 * practice run for range searching
 */
int
...
print_all_node_values_helper( KD_Node* node )
{
    ...
    {
        printf("This should literally never happen.\n");
    }
}
...
free_nodes(KD_Node* head)
{
    ...
    // I am new to c so please be kind.
}
...
KD_Node* construct_kd_tree_helper( int points[][DIMENSIONS], int num_points, int split_dimension)
{
    ...
    for (i = 0; i < num_points; i++)
    {
        ...
        // NOTE The < is actually very important. An 
        // inconsistency could mess with finding nodes.
        if (current < median)
        {
    ...
    // This makes freeing slightly easier.
    if ( !(*node).is_a_leaf )
    {
        (*node).location = NULL;
    }
